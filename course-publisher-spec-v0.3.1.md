# Course Publisher — техническая спецификация v0.3.1
...
## Patch v0.3.1 — вводный блок о профессии аналитика

Пайплайн, роли агентов, state machine, schemas и review-модель v0.3 не меняются.

Изменяется только стартовая часть Curriculum:

- курс начинается с общей карты аналитических ролей в IT;
- отдельно вводятся Business Analyst и System Analyst;
- подчёркивается, что граница между БА и СА зависит от компании и проекта и часто размыта;
- на курсе студент сознательно пробует обе стороны работы: от бизнес-проблемы до системной спецификации;
- названия должностей не используются как жёсткая граница ответственности;
- существующие стабильные `topic_id` сохраняются там, где тема уже существовала.

Новый Модуль 1 содержит 8 тем. Вместе с 3 темами Модуля 2 это сохраняет текущую удобную нумерацию:

```text
REQ-INTRO        -> course_order 12
REQ-FNFR         -> course_order 13
REQ-STAKEHOLDERS -> course_order 14
```

То есть тестовый сценарий Publisher с лекцией №14 не требует изменения.
...
modules:
  - id: M01
    title: Профессия аналитика в IT
    order: 1

    sections:
      - id: M01-S01
        title: Кто такие аналитики
        order: 1

        topics:
          - id: PROF-ANALYST-TYPES
            title: Какие бывают аналитики в IT
            order: 1
            course_order: 1

          - id: PROF-BA-SA
            title: Бизнес-аналитик и системный аналитик — в чём разница
            order: 2
            course_order: 2

          - id: PROF-BA-SA-BOUNDARY
            title: Где проходит граница между БА и СА и почему на практике она размыта
            order: 3
            course_order: 3

      - id: M01-S02
        title: Работа аналитика
        order: 2
        ...
        topics:
          ...
          - id: PROF-SKILLS
            title: Навыки и компетенции аналитика
            order: 3
            course_order: 6

      - id: M01-S03
        title: Наш путь на курсе
        order: 3

        topics:
          - id: PROF-BA-SA-PATH
            title: От бизнес-проблемы до системной спецификации
            order: 1
            course_order: 7

          - id: PROF-ARTIFACTS
            title: Какие результаты и артефакты возникают по пути
            order: 2
            course_order: 8
...
principles:
  ...
  - business_and_system_analysis_are_both_in_scope
  - analyst_role_boundaries_are_contextual
  - job_title_does_not_define_all_responsibilities
...
Курс рассчитан на человека без опыта работы аналитиком в IT.

Курс фокусируется на связке Business Analysis + System Analysis.
Студент проходит обе стороны работы: от понимания бизнес-проблемы,
целей, заинтересованных лиц и требований до моделей, данных,
интеграций и системной спецификации.

Важно не формировать у студента ложную жёсткую границу между БА и СА:
в разных компаниях обязанности распределяются по-разному,
а одна роль нередко совмещает задачи обоих направлений.
...
# 88. Baseline Curriculum Draft v0.3.1
...
```text
Модуль 1. Профессия аналитика в IT
├── Раздел 1. Кто такие аналитики
│   ├── Какие бывают аналитики в IT
│   ├── Бизнес-аналитик и системный аналитик — в чём разница
│   └── Где проходит граница между БА и СА и почему на практике она размыта
│
├── Раздел 2. Работа аналитика
│   ├── Типовые задачи бизнес- и системного аналитика
│   ├── Жизненный цикл работы аналитика в проекте
│   └── Навыки и компетенции аналитика
│
└── Раздел 3. Наш путь на курсе
    ├── От бизнес-проблемы до системной спецификации
...
### Авторская рамка для Модуля 1

Первая тема курса должна дать новичку карту профессии, а не сразу назначить ему одну должность.
...
Business Analyst ↔ System Analyst
    ↓
граница зависит от организации и проекта
    ↓
один человек часто совмещает обе группы задач
    ↓
на курсе проходим путь БА → СА целиком
```

При генерации вводных лекций необходимо объяснить, что кроме БА и СА существуют и другие аналитические специализации, например:
...
Они нужны как карта профессии, но не должны уводить курс в обучение этим специальностям.

Основной фокус курса:

> Понять бизнес-проблему и требования, а затем уметь довести их до достаточной системной спецификации.

Не следует учить студента универсальному правилу вида «БА всегда делает X, а СА всегда делает Y».
Корректнее показывать типичное разделение ответственности и сразу оговаривать, что на практике оно зависит от компании, команды и конкретного проекта.
...
## Применение patch v0.3.1 к уже созданному репозиторию

Если Publisher уже реализован по v0.3, **не пересоздавать pipeline**.

Достаточно обновить содержательные данные:

1. `course/curriculum.yaml` — заменить структуру Модуля 1;
2. создать Topic Passports для новых тем:
   - `PROF-ANALYST-TYPES`;
   - `PROF-BA-SA`;
   - `PROF-BA-SA-BOUNDARY`;
   - `PROF-SKILLS`;
   - `PROF-BA-SA-PATH`;
   - `PROF-ARTIFACTS`;
3. сохранить существующие IDs:
   - `PROF-ROLE`;
   - `PROF-LIFECYCLE`;
4. обновить `audience.md` и общие course principles;
5. пересчитать `progress.yaml` обычным Course Progress updater;
6. не удалять уже созданные runs и пробные Lesson Packages.

Поскольку после patch Модуль 1 содержит 8 тем, а Модуль 2 — 3 темы,
текущие номера основных тем Модуля 3 могут оставаться:

```text
12. Что такое требования и какие они бывают
13. Функциональные и нефункциональные требования
14. Заинтересованные лица
15. Способы выявления требований
```

Если локальная реализация уже содержит иное фактическое количество тем,
источником истины остаётся `topic_id`, а `course_order` должен быть пересчитан,
не меняя стабильные идентификаторы и историю runs.
